...
export type Item = {id:ItemId;name:string;kind:'weapon'|'armor'|'ring'|'ammo'|'consumable'|'quest'|'material';
 /** Uretilemeyen esyanin KAYNAGI - arayuzde "nereden gelir" diye gosterilir.
  *  'satin': yalniz tuccardan. 'hikaye': yalniz bir kisiden/olaydan. */
 sadece?:'satin'|'hikaye';description:string;rarity:'Sıradan'|'Nadir'|'Eşsiz'|'Görev';icon:string;attack?:number;defense?:number;hp?:number;price:number;menzilli?:boolean;hiz?:number;okHasar?:number;atesBagisik?:boolean;altinKat?:number;canGoster?:boolean;menzil?:number;arkadan?:number;sersemlet?:number;okHiz?:number;yakar?:number;delici?:boolean;ceker?:boolean;zehir?:number;kulKalkan?:number;yavaslik?:number;oldurunceCan?:number;kurtarma?:number;alan?:number;iyilesme?:number;sprite?:string};
export const ITEMS:Record<ItemId,Item>={
 ...
 guard:{id:'guard',name:'Muhafız kılıcı',kind:'weapon',description:'Alf’in sözü kadar sağlam. +17 saldırı.',rarity:'Nadir',icon:'sword',attack:17,price:90,sadece:'satin'},
 ember:{id:'ember',name:'Köz kılıcı',kind:'weapon',description:'+23 saldırı. Vuruşlar 3 saniye boyunca yakar.',rarity:'Eşsiz',icon:'flame',attack:23,price:160,sadece:'satin'},
 blood:{id:'blood',name:'Gece dişi',kind:'weapon',description:'+19 saldırı. Her vuruşta 3 can yeniler.',rarity:'Eşsiz',icon:'sword',attack:19,price:140,sadece:'satin'},
 bow:{id:'bow',name:'Avcı yayı',kind:'weapon',description:'+18 saldırı. Menzilli ok fırlatır.',rarity:'Nadir',icon:'bow',attack:18,price:95,menzilli:true,sadece:'satin'},
 tatar:{id:'tatar',name:'Tatar yayı',kind:'weapon',description:'+14 saldırı. Avcı yayından çok daha hızlı atar.',rarity:'Nadir',icon:'bow',attack:14,price:95,menzilli:true,hiz:.6,sadece:'satin'},
 ...
 yemin:{id:'yemin',name:'Yemin kılıcı',kind:'weapon',description:'+21 saldırı. Rauf yaşıyorsa 6 saldırı daha; yemin iki kişiyi de taşır.',rarity:'Eşsiz',icon:'sword',attack:21,price:120,sadece:'hikaye'},
 ...
 ash:{id:'ash',name:'Kül zırhı',kind:'armor',description:'+8 savunma, +20 azami can.',rarity:'Eşsiz',icon:'shield',defense:8,hp:20,price:150,sadece:'satin'},
 copper:{id:'copper',name:'Bakır yüzük',kind:'ring',description:'+3 saldırı. İçinde küçük bir yemin saklı.',rarity:'Sıradan',icon:'ring',attack:3,price:45,sadece:'satin'},
 life:{id:'life',name:'Yaşam halkası',kind:'ring',description:'+25 azami can.',rarity:'Nadir',icon:'heart',hp:25,price:120,sadece:'satin'},
 wind:{id:'wind',name:'Rüzgâr mührü',kind:'ring',description:'Kaçınma 0,6 saniye daha hızlı dolar.',rarity:'Nadir',icon:'wind',price:95,sadece:'satin'},
 kanm:{id:'kanm',name:'Kan mührü',kind:'ring',description:'Öldürdüğün her düşman sana 8 can verir.',rarity:'Eşsiz',icon:'heart',price:90,oldurunceCan:8,sadece:'hikaye'},
 yeminh:{id:'yeminh',name:'Yemin halkası',kind:'ring',description:'Canın üçte birin altına düşerse bir kez 40 can dolar. Her bölgede yenilenir.',rarity:'Eşsiz',icon:'ring',price:100,kurtarma:40,sadece:'hikaye'},
 gozu:{id:'gozu',name:'Kül gözü',kind:'ring',description:'Düşmanların canını görürsün. Bulduğun altın %10 artar.',rarity:'Nadir',icon:'gem',price:85,altinKat:1.1,canGoster:true,sadece:'satin'},
 ...
 *  tarif "odun + altin" istiyordu - hammadde sistemi gelince anlamsiz kaldi.
 *  Tasarim kurallari:
 *   * Her tarif EN AZ bir hammadde ister; altin yalniz "satin alinan parca"
 *     gereken birkac tarifte var (cam, tuz, ip gibi seyleri sigginaktan
 *     aliyorsun sayiliyor).
 *   * Hammadde tek bir dusman turune baglandigi icin tarif de dolayli olarak
 *     "su yaratigi kes" gorevi haline geliyor - celik yalniz Muhafiz/Bekci'den,
 *     zehir kesesi yalniz orumcekten.
 *   * URETILEMEYENLER bilincli: yay ve varyantlari, iyi kiliclar, yuzukler ve
 *     hikaye esyalari. Bunlar tuccardan ya da hikayeden gelir (bkz. Item.sadece).
 */
export type Tarif={id:ItemId;adet:number;malzeme:Partial<Record<ItemId,number>>;gold?:number};
export const TARIFLER:Tarif[]=[
 /* --- tuketilebilir --- */
 {id:'torch',  adet:2, malzeme:{wood:1,pacavra:1}},
 {id:'arrow',  adet:10,malzeme:{wood:1,kemikp:1}},
 {id:'okzehir',adet:5, malzeme:{zehirk:1,wood:1}},
 {id:'okates', adet:5, malzeme:{kurum:2,wood:1}},
 {id:'tuzet',  adet:1, malzeme:{et:2,kurum:1}},
 {id:'durusu', adet:1, malzeme:{kanat:1,kurum:2}},
 {id:'merhem', adet:1, malzeme:{pacavra:2,post:1}},
 {id:'potion', adet:1, malzeme:{et:1,pacavra:1},gold:8},
 {id:'toz',    adet:1, malzeme:{kurum:3,ag:1}},
 {id:'tonic',  adet:1, malzeme:{zehirk:1,kurum:2},gold:12},
 /* --- zirh --- */
 {id:'yelek',  adet:1, malzeme:{post:4,ag:2}},
 {id:'kemik',  adet:1, malzeme:{kemikp:6,post:2,ag:2}},
 {id:'chain',  adet:1, malzeme:{celik:3,pacavra:2}},
 {id:'ocakz',  adet:1, malzeme:{celik:5,kurum:4},gold:30},
 /* --- silah --- */
 {id:'hancer', adet:1, malzeme:{celik:1,post:1}},
...
export function tarifDurum(s:State,t:Tarif){
 const eksik:{id:ItemId;gerek:number;var:number}[]=[];
 for(const [id,n] of Object.entries(t.malzeme)as [ItemId,number][]){
  const v=s.inventory[id]||0;if(v<n)eksik.push({id,gerek:n,var:v});
 }
 const altinYeter=s.gold>=(t.gold||0);
 /* Tuketilebilir olmayan bir esyadan ikincisi uretilemez - kusanilir esya
    zaten tek; ikinci kopya envanteri sisiriyordu. */
 const zaten=ITEMS[t.id].kind!=='consumable'&&ITEMS[t.id].kind!=='ammo'&&!!s.inventory[t.id];
 return {olur:!eksik.length&&altinYeter&&!zaten,eksik,altinYeter,zaten};
}
/** Tarifi uygular. Cagirmadan once tarifDurum().olur kontrol edilmeli. */
export function tarifUygula(s:State,t:Tarif){
 if(!tarifDurum(s,t).olur)return false;
 for(const [id,n] of Object.entries(t.malzeme)as [ItemId,number][]){
  s.inventory[id]=(s.inventory[id]||0)-n;if(s.inventory[id]!<=0)delete s.inventory[id];
 }
...
export const GUN_OLAYLARI:Record<number,GunOlay>={
 /* 5. GUN - "Kosedeki adam". Ortada KATIL YOK: kral kendi eliyle gitti,
    tacini sonradan biri aldi. Ama sigginak bir fail ariyor ve oyuncunun
    sucladigi kisi gercekten bedel oduyor - asil trajedi bu.
    Kosul: oyuncu krali gormus olmali. Gormemisse olay ERTELENIR (bkz. gunGec). */
 5:{id:'kralOldu',baslik:'Köşedeki adam ölmüş.',
  metin:'Sabah köşede kimse kıpırdamıyordu. Kral ölmüş; başındaki paslı taç yok. Sığınak bir isim arıyor.',
  kosul:s=>!!s.flags.kralGoruldu,
  uygula:s=>{s.flags.kral='oldu';
   /* GERCEK: kendi eli. Tac ayri bir mesele - cesedi ilk bulan aldi. */
   s.flags.kralSon='kendi';s.flags.kralTac='karga';s.flags.kralCinayet='acik';}},
};
...
export function stats(s:State){const weapon=ITEMS[s.equipment.weapon],armor=ITEMS[s.equipment.armor],ring=s.equipment.ring?ITEMS[s.equipment.ring]:null;/* DENGE (v16.1). Olculdu: oyuncu dusmanlardan cok daha hizli gucleniyordu -
   seviye 5 + Koz kilici ile fare ve yarasa TEK vurusta oluyor, oyuncunun
   olmesi 17 vurus suruyordu. Silah 10->23 zaten 2.3 kat; ustune seviye ve
   yetenek de eklenince dusman cani sabit kaldigi icin oyun erimeye
   basliyordu. Seviye katkisi 2->1.5, guc yetenegi 4->3, can katkilari
   12->8 ve 18->14: ilerleme hala hissediliyor ama dusmani gecersiz
   kilmiyor. Dusman canlari da ayrica yukseltildi (bkz. engine.ts CAN). */
return {maxHp:100+(s.level-1)*8+s.skills.vigor*14+(armor.hp||0)+(ring?.hp||0),attack:(weapon.attack||0)+(s.equipment.weapon==='yemin'&&Number(s.flags.raufCan||0)>0?6:0)+(ring?.attack||0)+Math.round((s.level-1)*1.5)+s.skills.power*3,defense:armor.defense||0,dodge:Math.max(.65,2.2-s.skills.agility*.25-(s.equipment.ring==='wind'?.6:0))}}
...
export function buy(s:State,id:ItemId,satici='boran'){const item=ITEMS[id];const v=SATICILAR[satici];if(!v||!v.liste.includes(id))return false;
 /* Kral cinayetinde suclanan tuccar bir daha satmaz (bkz. karar_* eylemleri). */
...
/** UC FINAL. Kullanici uc fikri de "alternatif final" olarak istedi:
 *  gocmek, felaketin sebebini bulmak, disaridaki seyi avlamak.
 *  Her finalin GORUNUR adimlari var - oyuncu neye dogru calistigini bilmeli,
 *  yoksa "hikayeyi ogrenmek icin oynamaya devam etme" istegi karsilanmaz.
 *  Adimlarin cogu ZATEN VAR OLAN bayraklara baglandi; yeni sistem degil,
 *  dagilmis parcalarin toplanmasi. */
export type FinalAdim={id:string;baslik:string;ipucu:string;tamam:(s:State)=>boolean};
export type Final={id:'goc'|'gercek'|'av';ad:string;ozet:string;adimlar:FinalAdim[]};
export const FINALLER:Final[]=[
 {id:'goc',ad:'Göç',ozet:'Buradakileri toplayıp başka bir yere götürmek. Kimseyi geride bırakmadan.',
  adimlar:[
   {id:'goc_yasat',baslik:'Kimseyi kaybetme',ipucu:'Depo hiç boşalmasın; açlıktan kimse ölmesin.',tamam:s=>s.kayip===0&&s.gun>=4},
   {id:'goc_stok',baslik:'Yol için erzak',ipucu:'Depoda en az 8 gün yiyecek ve 8 gün su biriktir.',tamam:s=>s.erzak.yiyecek>=8&&s.erzak.su>=8},
   {id:'goc_yol',baslik:'Bir yol bul',ipucu:'Terk Edilmiş Koridor’un öbür ucunu gör.',tamam:s=>!!s.flags.gordu_test100},
   {id:'goc_soz',baslik:'İkna et',ipucu:'Mirna ve Alf sana güvensin; ikisinin de işini bitir.',tamam:s=>!!s.flags.medicineDone&&!!s.flags.ledgerDone},
  ]},
 {id:'gercek',ad:'Gerçek',ozet:'Külün nereden geldiğini öğrenmek. Öğrenmek kimseyi kurtarmaz, ama yalanı bitirir.',
  adimlar:[
   {id:'ger_kral',baslik:'Kralın hikâyesini topla',ipucu:'Beş kişiden de Kral’ı dinle.',tamam:s=>['kralMirna','kralAlf','kralUndur','kralLin','kralObruk'].every(k=>s.flags[k])},
   {id:'ger_kapi',baslik:'Kapının sırrı',ipucu:'Alf’in yemininin neye dayandığını öğren.',tamam:s=>s.flags.alfSir==='soylendi'},
   {id:'ger_asagi',baslik:'Aşağıya in',ipucu:'Dar Geçit’i ve Unutulmuş Sarnıç’ı gör.',tamam:s=>!!s.flags.gordu_tunel&&!!s.flags.gordu_cistern},
   {id:'ger_defter',baslik:'Deftere yazdır',ipucu:'Undur’a defteri ulaştır.',tamam:s=>!!s.flags.ledgerDone},
  ]},
 {id:'av',ad:'Av',ozet:'Dışarıyı tutan şeyi öldürmek. Kapı açılsın diye değil; bir daha kimse o sesi duymasın diye.',
  adimlar:[
   {id:'av_bekci',baslik:'Kül Bekçisi’ni devir',ipucu:'Sarnıcın dibindeki şeyi öldür.',tamam:s=>s.killed.includes('warden')},
   {id:'av_muhafiz',baslik:'Son Muhafız’ı geç',ipucu:'Ovadaki muhafızla işini bitir.',tamam:s=>!!s.flags.muhafiz},
   {id:'av_celik',baslik:'Kendi silahını yap',ipucu:'Çelik hurdasından bir silah üret.',tamam:s=>['hancer','mizrak','balta','topuz'].some(k=>!!s.inventory[k as ItemId])},
   {id:'av_zirh',baslik:'Ağır zırh kuşan',ipucu:'Zincir ya da ocak zırhını üret ve kuşan.',tamam:s=>['chain','ocakz','kemik'].includes(s.equipment.armor)},
  ]},
];
/** Her finalin ilerlemesi - gunluk panelinde gosterilir. */
export function finalDurum(s:State){
 return FINALLER.map(f=>{
  const adimlar=f.adimlar.map(a=>({...a,bitti:a.tamam(s)}));
  return {...f,adimlar,tamam:adimlar.filter(a=>a.bitti).length,toplam:adimlar.length,hazir:adimlar.every(a=>a.bitti)};
 });
}
...
export function sonMetni(s:State):string{
 ...
 if(s.ending==='goc')return 'Kapıyı arkanızdan kapatmadınız; kapanacak bir şey kalmamıştı. Koridorun öbür ucunda hava hâlâ ağırdı ama nefes alınıyordu. Mirna önde yürüdü, Alf en arkada. Sığınak boşaldı ve ilk kez birinin evi olmadı — bir yol oldu.'+ek;
 if(s.ending==='gercek')return 'Undur son sayfayı yazdı ve defteri kapatmadı. Külün nereden geldiğini artık biliyorsun; kimseyi kurtarmıyor, kimseyi diriltmiyor. Ama sığınakta bir daha kimse “bilmiyoruz” demeyecek. Bazen bir yalanın bitmesi de bir sondur.'+ek;
 if(s.ending==='av')return 'Sesi kesildiğinde kimse sevinmedi; herkes kulak kabarttı, bir daha gelir mi diye. Gelmedi. Kapıyı açtılar, dışarısı yine küldü ama artık yalnızca küldü. Alf kılıcını duvara astı ve ilk kez uyudu.'+ek;
...
export function dialogue(s:State,id:string):Dialogue{
 ...
 if(id==='nil'){
  return {who:'Lin',role:'Ateşi söndürmeyen',portrait:7,
   text:s.flags.nilSondu?'…Söndü. Bakma bana. Odun taşımak kolaydı; zor olan neden taşıdığını bilmekti. Sen onu aldın.'
    :s.flags.kral==='oldu'?'Kral amca gitti. Köz götürecek kimsem kalmadı. Ateşi yine söndürmem ama… kime götüreceğim onu?'
   ...
   choices:[...(s.flags.kralCinayet==='acik'&&!s.flags.ifadeLin?[{label:'Kral amcayı en son ne zaman gördün?',action:'ifade_lin',note:'Soruşturma · Lin'}]:[]),...(s.flags.nilSondu?[]:[{label:'Ağabeyin nerede?',action:'story:nil:1'}]),
 ...
 if(id==='obruk'){
  ...
  return {who:'Obruk',role:bey?'Kendi kapısının beyi':'Kileri dolu adam',portrait:10,
   ...
   choices:[...(s.flags.kralCinayet==='acik'&&!s.flags.ifadeObruk?[{label:'Ocağın üstündeki taç nereden geldi?',action:'ifade_obruk',note:'Soruşturma · Obruk'}]:[]),{label:'Konuşalım.',action:'story:obruk:1'},
 ...
 if(id==='karga')return {who:'Karga',role:'Obruk’un adamı',portrait:11,
  ...
  choices:[...(s.flags.kralCinayet==='acik'&&!s.flags.ifadeKarga?[{label:'O gece dışarıdaymışsın.',action:'ifade_karga',note:'Soruşturma · Karga'}]:[]),{label:'Konuşalım.',action:'story:karga:1'},close]};
 if(id==='cakal')return {who:'Çakal',role:'Obruk’un adamı',portrait:12,
  ...
  choices:[...(s.flags.kralCinayet==='acik'&&!s.flags.ifadeCakal?[{label:'O gece Karga neredeydi?',action:'ifade_cakal',note:'Soruşturma · Çakal'}]:[]),close]};
 if(id==='mira')return {who:'Mirna',role:'Sığınağın şifacısı',portrait:3,text:(s.flags.kral==='oldu'?(s.flags.kralKarar==='kendi'?'On dokuz. Yazdım. Yanına da senin cümleni yazdım. ':s.flags.kralKarar?'On dokuz. Yazdım. Bir isim daha yazdım; onu yazarken elim titredi. ':'On dokuz. Yazdım. Adının yanı boş duruyor. ') : '')+(s.flags.medicineDone?(s.flags.medicine==='rauf'?'Bir hayat kurtardın. Buradakiler için başka bir yol bulacağız. Yaralarını sarayım.':'İlaç işe yaradı. Bu gece kimseyi kaybetmedik. Dinlen; yaralarını sarayım.'):s.flags.medicine==='rauf'?'Ellerin boş… ama yüzünde söylemek istediğin bir şey var.':s.inventory.medicine?'Buldun! Bir şişenin bu kadar ağır bir umut taşıyacağını düşünmezdim.':'Aşağıdaki sarnıçta bir doz ilaç kaldı. Burada ateşler içinde yatanlar var. Onu bana getirir misin?'),choices:[
  /* Kral cinayeti: ifade + KARAR. Karar Mirna'ya veriliyor cunku olen herkesi
     o yaziyor - "kim" sorusunun muhatabi o. Karar icin en az bir ifade sart;
     'kendi eliyle' secenegi ise YALNIZCA Mirna'nin ifadesi alindiysa acilir,
     cunku bunu bilen tek kisi cesedi yikayan kisidir. */
  ...(s.flags.kralCinayet==='acik'&&!s.flags.ifadeMirna?[{label:'Cesedi sen yıkadın. Ne gördün?',action:'ifade_mirna',note:'Soruşturma · Mirna'}]:[]),
  ...(s.flags.kralCinayet==='acik'&&['ifadeMirna','ifadeAlf','ifadeObruk','ifadeKarga','ifadeCakal','ifadeLin'].some(k=>s.flags[k])?[
   ...(s.flags.ifadeMirna?[{label:'Kimse öldürmedi. Kendi eliyle gitti.',action:'karar_kendi',note:'Karar · Köşedeki adam'}]:[]),
   {label:'Karga aldı tacı. O öldürdü.',action:'karar_karga',note:'Karar · Suçlama'},
 ...
 if(id==='ekin')return {who:'Undur',role:'Yeminlerin arşivcisi',portrait:4,text:(s.flags.kral==='oldu'?'Ongun. Bereket. Altına çizgi çektim. Tacın nerede olduğunu biliyorum; onu da yazdım. ':'')+(s.ending?(s.flags.alfSir==='soylendi'?'Bir yemin, onu tutan insanlar kadar güçlüdür. Alf’inki yalanmış; sen söyledin, o da bıraktı. Bunu yazdım.':s.flags.alfSir==='sakladin'?'Bir yemin, onu tutan insanlar kadar güçlüdür. Alf hâlâ kendi yükünü taşıyor — sayende. Bunu da yazdım.':'Bir yemin, onu tutan insanlar kadar güçlüdür. Seninkinin izini bu taşlar uzun süre taşıyacak.'):'Sarsıntılar artıyor. Aşağıda, sarnıcın da altında bir yer var — eski kalp orada atıyor. Yolunu henüz bulamadım. Bulduğumda sana ilk ben söylerim.'),choices:[
  /* UC FINAL. Yalnizca ADIMLARI TAMAMLANMIS olanlar gorunur - oyuncu hazir
     olmadigi bir sonu secemez, ama hangi sonlara yaklastigini gunlukten gorur. */
  ...(!s.ending?finalDurum(s).filter(f=>f.hazir).map(f=>({label:`${f.ad}: ${f.ozet}`,action:'final_'+f.id,note:'SON · geri dönüşü yok'})):[]),
{label:'Bana hikâyeni anlat.',action:'story:ekin:1'},close]};
...
export function choose(s:State,action:string):{message:string;special?:'close'|'shop'|'ending';leveled?:boolean;satici?:string}{
 ...
 /* Final secimi. Geri donusu yok; adimlar zaten tamamlanmis olmali. */
 case 'final_goc':case 'final_gercek':case 'final_av':{
  if(s.ending)return {message:''};
  const f=action.slice(6) as 'goc'|'gercek'|'av';
  const d=finalDurum(s).find(x=>x.id===f);
  if(!d?.hazir)return {message:'Henüz değil. Eksiklerini günlükten görebilirsin.'};
  s.ending=f;gainXp(s,200);
 ...
 /* ---- KRAL CINAYETI: IFADELER ----
    ...
    Yalnizca Mirna'nin ifadesi "olum" sorusunu cevapliyor; digerleri "tac"
    sorusunu cevapliyor ve oyuncu ikisini ayni sey saniyor. */
 case 'ifade_mirna':if(s.flags.ifadeMirna)return {message:''};s.flags.ifadeMirna='alindi';xp=25;
  message='Mirna: “Onu ben yıkadım. Tek bir yara vardı. Yeri… elinin rahatça uzanabileceği bir yerdi. Bunu sana söyledim; kime söyleyeceğini sen bil.”';break;
 case 'ifade_alf':if(s.flags.ifadeAlf)return {message:''};s.flags.ifadeAlf='alindi';xp=15;
  message='Alf: “Kapıdaydım. Hep kapıdayım. …Ama sorun o değil. On bir yıl önce açma emrini o verdi, kolu ben çektim. Hesabı bu sabah ödedi. İstersen adımı yaz.”';break;
 case 'ifade_obruk':if(s.flags.ifadeObruk)return {message:''};s.flags.ifadeObruk='alindi';s.flags.tac='satildi';xp=20;
  message='Obruk: “Aldım, evet. Ocağın üstünde duruyor. Parasını da saydım. Kimden aldığımı sormuyorsun, akıllısın; ben de söylemem. Mal maldır, ölü ölüdür, ikisi ayrı hesap.”';break;
 case 'ifade_karga':if(s.flags.ifadeKarga)return {message:''};s.flags.ifadeKarga='alindi';xp=20;
  message='Karga: “…Dışarıdaydım. Konuşuyorum ya, çok konuşuyorum. Bir şey gördüm, aldım. Almasaydım başkası alırdı. Ben bir şey YAPMADIM. …Aldım, o kadar.”';break;
 case 'ifade_cakal':if(s.flags.ifadeCakal)return {message:''};s.flags.ifadeCakal='alindi';xp=15;
  message='Çakal: “Geç döndü. Ceketinin altında bir şey vardı ve elleri titriyordu. Ben konuşmam demiştim; bunu da konuşmadım sayılır.”';break;
 case 'ifade_lin':if(s.flags.ifadeLin)return {message:''};s.flags.ifadeLin='alindi';xp=15;
  message='Lin: “Akşam bana baktı. Hiç bakmazdı. Sonra tacını çıkardı, dizine koydu. Ben köz götürüyordum, geri dönmedim. …Dönseydim?”';break;
 /* ---- KRAL CINAYETI: KARAR ----
    Tek DOGRU cevap 'kendi', ama oyun onu odullendirip digerlerini cezalandirmiyor;
    her karar birinin hayatini degistiriyor ve oyuncu bununla kaliyor. */
 case 'karar_kendi':if(s.flags.kralKarar)return {message:''};s.flags.kralKarar='kendi';s.flags.kralCinayet='kapandi';xp=90;
  message='Mirna uzun uzun baktı: “…Yazdım. ‘Kendi eliyle’ diye yazdım. Sığınak bir isim istiyordu, sen bir cümle verdin. Cümle daha ağır.” Kimse cezalandırılmadı. Kimse rahatlamadı da.';break;
 case 'karar_karga':if(s.flags.kralKarar)return {message:''};s.flags.kralKarar='karga';s.flags.kralCinayet='kapandi';s.flags.kargaSurgun='evet';xp=40;
  message='Karga’yı kapıya çıkardılar. Bağırmadı; “Ben bir şey yapmadım, aldım” dedi, kimse dinlemedi. Doğruydu: almıştı. Öldürmemişti.';break;
 case 'karar_obruk':if(s.flags.kralKarar)return {message:''};s.flags.kralKarar='obruk';s.flags.kralCinayet='kapandi';s.flags.obrukKapali='evet';xp=30;
  message='Obruk kilerinin kapısını yüzüne kapattı: “Bir daha benden bir çöp alamazsın.” Adamları hâlâ yanında. Taç hâlâ ocağın üstünde.';break;
 case 'karar_alf':if(s.flags.kralKarar)return {message:''};s.flags.kralKarar='alf';s.flags.kralCinayet='kapandi';s.flags.alfKirildi='evet';xp=30;
  /* Alf DUNYADAN CIKARILMIYOR: "defter" gorevi onunla kapaniyor, cikarsaydi
     oyun kilitlenirdi. Bedel iliskide: ticaret kapaniyor, sesi soguyor. */
