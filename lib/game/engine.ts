...
if(!walkable(this.world,s.x,s.y)){[s.x,s.y]=this.world.spawn;}this.camera={x:s.x-this.gorus.en/2,y:s.y-this.gorus.boy/2};this.resetMobs();this.attackTimer=this.dodgeTimer=this.dash=this.invulnerable=this.tonic=0;this.input={x:0,y:0,attack:false};this.audio.setZone(s.zone);this.emit();}
 ...
 private attack(){if(this.attackTimer>0||this.paused)return;const s=this.state;const isBow=s.equipment.weapon==='bow';this.attackTimer=isBow?.38:.43;this.vurusSure=this.attackTimer;this.vurusPoz=this.attackTimer;
 ...
 private move(p:{x:number;y:number},dx:number,dy:number){
  ...
  this.tick+=dt;this.state.playtime+=dt;for(const key of ['attackTimer','dodgeTimer','dash','invulnerable','tonic','slash','vurusPoz','trapCooldown']as const)this[key]=Math.max(0,this[key]-dt);
  ...
  *  koyu renkte 1px kaydirilarak dort yone cizilir, uzerine asil renk gelir. */
  ...
  if(!dusuyor)this.sprite(this.kit()+this.direction+action,s.x,s.y,action==='Walk'?Math.floor(this.yol/Engine.ADIM):action==='Attack'?Math.floor((this.vurusSure-this.vurusPoz)*16):Math.floor(time*5),Engine.OYUNCU_EN,32,this.direction==='S'&&this.flip,OYUNCU_OLCEK,this.invulnerable>0&&Math.floor(time*18)%2===0?.45:1);if(this.slash>0){c.strokeStyle='#f5db9ac9';c.lineWidth=1.5;const angle=this.direction==='S'?(this.flip?Math.PI:0):this.direction==='U'?-Math.PI/2:Math.PI/2;c.beginPath();c.arc(s.x,s.y-5*OYUNCU_OLCEK,23*OYUNCU_OLCEK,angle-1.1,angle+1.1);c.stroke();}}});
