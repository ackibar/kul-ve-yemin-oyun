...
for(const [key,name]of [['fire','Fire1'],['lever','Lever1'],['trap','Spikes']])jobs.push(this.img(key,`/assets/dungeon/3%20Animated%20objects/${name}.png`));/* Sandik artik CraftPix setinden degil: oyunun paletinde uretilmis iki
   kareli kendi sheet'i (0 kapali, 1 acik). */jobs.push(this.img('chest','/assets/nesne/sandik.png'));for(const p of ['sol','sag'])jobs.push(this.img('perde_'+p,`/assets/nesne/perde_${p}.png`));for(const a of ["camasir", "fener", "fici", "kasa", "masa", "ocak", "odun", "raf", "sandik", "tabure", "tezgah", "yatak1", "yatak2"])jobs.push(this.img('nesne/'+a+'.png',`/assets/nesne/${a}.png`));mark(false);const result=await Promise.allSettled(jobs);
...
if(!walkable(this.world,s.x,s.y)){[s.x,s.y]=this.world.spawn;}this.camera={x:s.x-this.gorus.en/2,y:s.y-this.gorus.boy/2};this.resetMobs();/* Mesale kayitta kaldiysa yanmaya devam eder. */this.mesale=Number(this.state.flags.mesaleKalan||0);this.attackTimer=this.dodgeTimer=this.dash=this.invulnerable=this.tonic=0;this.input={x:0,y:0,attack:false};this.audio.setZone(s.zone);this.emit();}
  ...
  *  koyu renkte 1px kaydirilarak dort yone cizilir, uzerine asil renk gelir. */
  ...
  /* Perdeler aktor listesinde: oyuncunun y'sine gore siralandiklari icin
     perdenin arkasina gecen oyuncu bezin ARDINDA kalir, onunde duran onunde.
     Cizim capasi bezin DIP cizgisi (e.y); gorsel oradan yukari uzanir. */
  for(const e of this.world.entities)if(e.type==='perde')actors.push({y:e.y,draw:()=>{
   const im=this.images[e.asset||'perde_sol'];if(!im?.naturalWidth)return;
   const w=im.naturalWidth/R,h=im.naturalHeight/R;
   c.drawImage(im,0,0,im.naturalWidth,im.naturalHeight,Math.round(e.x-w/2),Math.round(e.y-h),w,h);}});
