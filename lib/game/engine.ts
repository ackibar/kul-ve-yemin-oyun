...
if(!walkable(this.world,s.x,s.y)){[s.x,s.y]=this.world.spawn;}this.camera={x:s.x-this.gorus.en/2,y:s.y-this.gorus.boy/2};this.resetMobs();this.attackTimer=this.dodgeTimer=this.dash=this.invulnerable=this.tonic=0;this.input={x:0,y:0,attack:false};this.audio.setZone(s.zone);this.emit();}
 ...
 private kit(){const w=this.state.equipment.weapon;const v=w==='bow'?'bow':w==='yumruk'?'':'sword';/* Kontrol daima 'D' uzerinden: capraz yonde sheet olmayabilir ve set varken yedege dusmek yanlis olur. */return this.images['characters1'+v+'DIdle']?.naturalWidth?'characters1'+v:'characters1';}
 ...
 toggleWeapon(){if(this.paused||!this.ready)return;const s=this.state;
  const kilic=(Object.keys(s.inventory)as ItemId[]).filter(id=>ITEMS[id].kind==='weapon'&&id!=='bow'&&id!=='yumruk')
   .sort((a,b)=>(ITEMS[b].attack||0)-(ITEMS[a].attack||0))[0];
  const sira:ItemId[]=[];if(kilic)sira.push(kilic);if(s.inventory.bow)sira.push('bow');sira.push('yumruk');
  const next=sira[(sira.indexOf(s.equipment.weapon)+1)%sira.length];
  this.notify(next==='bow'?`Avcı yayı kuşanıldı (Menzilli ok modu). Kalan ok: ${s.inventory.arrow||0}`
