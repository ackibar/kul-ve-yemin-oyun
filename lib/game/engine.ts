...
/** Yerdeki ganimet. 'esya' turu GENEL: hangi esya oldugunu `item` tasiyor -
 *  her hammadde icin ayri bir kind acmak yerine tek dal. */
export type Drop={id:string;x:number;y:number;kind:'wood'|'xp'|'gold'|'bow'|'esya';item?:ItemId;amount:number;vx:number;vy:number;life:number};
...
export class Engine{
 ...
 /** Iskelet ailesi: 11 sıradan, 12 AZMAN. Ikisi de gomulu baslar, ikisi de
  *  olunce kemige dagilir; farklari can, boy ve tek vurusta olup olmamalari. */
 static readonly ISKELET=(k:number)=>k===11||k===12;
...
mark(false);const result=await Promise.allSettled(jobs);
 ...
 private resetMobs(){const diri=this.world.enemies.filter(e=>!this.state.killed.includes(e.id))
  ...
  this.gomulu=diri.filter(e=>Engine.ISKELET(e.kind));
  this.mobs=diri.filter(e=>!Engine.ISKELET(e.kind)).map(e=>{const max=e.boss?300:(Engine.CAN[e.kind]??40);return {...e,hp:max,max,cool:1+Math.random(),windup:0,burn:0,hurt:0,homeX:e.x,homeY:e.y}});this.cikisBekle={};this.shots=[];this.particles=[];this.drops=[];this.activeTraps.clear();}
...
if(!walkable(this.world,s.x,s.y)){[s.x,s.y]=this.world.spawn;}this.camera={x:s.x-this.gorus.en/2,y:s.y-this.gorus.boy/2};this.izler=[];this.resetMobs();/* Mesale kayitta kaldiysa yanmaya devam eder. */this.mesale=Number(this.state.flags.mesaleKalan||0);this.attackTimer=this.dodgeTimer=this.dash=this.invulnerable=this.tonic=this.dusus=0;this.dustu=false;this.input={x:0,y:0,attack:false};this.audio.setZone(s.zone);this.emit();}
 ...
 spawnDrop(x:number,y:number,kind:Drop['kind'],amount:number,item?:ItemId){this.drops.push({id:`drop_${Date.now()}_${Math.random()}`,x,y,kind,item,amount,vx:(Math.random()-.5)*45,vy:(Math.random()-.6)*45,life:30});}
 ...
 private attack(){if(this.attackTimer>0||this.paused)return;const s=this.state;const silah=ITEMS[s.equipment.weapon];const isBow=silah.menzilli===true;/* Sure artik silahin kendi `hiz` carpanindan ve bileme tasindan geliyor;
   ...
   this.slash=Engine.SLASH_SURE;this.slashIsabet=false;this.vurusSetSec();/* Ciplak elde kilic savurmasi calmiyor: metalik tini yumrukta yanlis. */this.audio.play(s.equipment.weapon==='yumruk'?'yumrukSavur':'swing');
   ...
   (ic carpim pozitif) sirtini donmus demektir. */if(silah.arkadan&&m.aci!==undefined){const vx=m.x-s.x,vy=m.y-s.y,n=Math.hypot(vx,vy)||1;if((Math.cos(m.aci)*vx+Math.sin(m.aci)*vy)/n>.35){damage=Math.round(damage*silah.arkadan);this.float(m.x,m.y-22,'SIRTTAN!','#ffd1a3');}}m.hp-=damage;m.hurt=Engine.HURT_SURE;if(silah.sersemlet)m.sersem=silah.sersemlet;if(s.equipment.weapon==='ember')m.burn=3;if(silah.yakar)m.burn=Math.max(m.burn,silah.yakar);if(s.equipment.weapon==='blood')s.hp=Math.min(stats(s).maxHp,s.hp+3);this.float(m.x,m.y-12,String(damage),'#ffdaa3');{const dd=Math.hypot(m.x-s.x,m.y-s.y)||1;this.darbeKivilcim(m,(m.x-s.x)/dd,(m.y-s.y)/dd);}/* Dusmana vurus sesi KALDIRILDI (kullanici: "8bit bir ses cikiyor"). 'hit' kare dalga sentezi - tahta esya kirmada hala kullaniliyor ama dusmana her vurusta calinca oyunun geri kalanindan kopuk duruyordu. Yerine kayitli bir darbe gelirse ORNEKLER'e yeni bir ad eklenir. */const d=Math.hypot(m.x-s.x,m.y-s.y)||1;this.move(m,(m.x-s.x)/d*Engine.KILIC_ITME,(m.y-s.y)/d*Engine.KILIC_ITME);if(m.id==='rauf'&&m.hp<=m.max*.18){this.raufDizCok();}else if(m.hp<=0)this.kill(m);}/* ISABET SESI - dongunun DISINDA, savurus basina TEK kez. Yarma baltasi (alan:3) uc hedefe birden vuruyor; ses uc kez calarsa master kompresoru muzigi de birlikte eziyor. 'swing' iskada da caliyor, bu ses yalnizca isabette biniyor - iska ile isabetin ayrimi tam olarak bu. */if(vurdu){this.slashIsabet=true;this.audio.play(s.equipment.weapon==='yumruk'?'yumruk':'vurus');const hedef=targets[0],hd=Math.hypot(hedef.x-s.x,hedef.y-s.y)||1;const ux=(hedef.x-s.x)/hd,uy=(hedef.y-s.y)/hd;/* Oldurucu darbe iki kat donar - son vurus ayri okunsun. */this.donma=Math.max(this.donma,Engine.DARBE_DONMA*(silah.hiz??1)*(hedef.hp<=0?2:1));this.sarsinti=Engine.SARSINTI_SURE;this.sarsintiYon={x:ux,y:uy};this.vurusIleri=Engine.VURUS_ILERI_SURE;this.vurusIleriYon={x:ux,y:uy};}this.emit();}
 ...
 private dashSiyir(){
  ...
  for(const m of this.mobs){
   ...
   m.hp-=Engine.DASH_HASAR;m.hurt=Engine.HURT_SURE;
   const it=m.boss?6:Engine.DASH_ITME;
   this.move(m,dx/d*it,dy/d*it);
   this.float(m.x,m.y-12,String(Engine.DASH_HASAR),'#ffdaa3');
   this.burst(m.x,m.y,'#c9b7a3',6);
   if(m.id==='rauf'&&m.hp<=m.max*.18)this.raufDizCok();else if(m.hp<=0)this.kill(m);
  }
 }
 /** Dusman turune gore HAMMADDE dusurur (bkz. data.ts DUSURME).
  *  Parcalar dusmanin cevresine sacilir ki tek bir yigin olarak ust uste
  *  binmesinler - toplama yaricapi 65 birim, hepsi zaten kendiliginden gelir. */
 private ganimetDus(m:Mob){
  for(const g of DUSURME[m.kind]??[]){
   if(Math.random()>=g.sans)continue;
   const adet=g.az+Math.floor(Math.random()*(g.cok-g.az+1));
   this.spawnDrop(m.x+(Math.random()-.5)*10,m.y+(Math.random()-.5)*8,'esya',adet,g.id);
  }
 }
 private kill(m:Mob){
  ...
  if(Engine.ISKELET(m.kind)){
   ...
   this.iskeletDagit(m,this.dusmanYon(m));this.ganimetDus(m);
   /* Siradan iskelet: yalniz 6 XP, altin YOK ve save() YOK - 40 kisilik
      kalabalikta servet ve onlarca yazma olurdu. AZMAN farkli: tek tek
      oldurulen, kayda deger bir dusman. */
   if(m.kind===12){this.spawnDrop(m.x,m.y,'xp',55);
    this.spawnDrop(m.x+(Math.random()-.5)*12,m.y+(Math.random()-.5)*12,'gold',14);
    this.save();}
   else this.spawnDrop(m.x,m.y,'xp',6);
   return;
  }
  if(this.state.killed.includes(m.id))return;this.state.killed.push(m.id);
 ...
 private dusmanPoz(kind:number,dir:string,action:string){
  ...
  if(kind===12)kind=11;
  const k=`enemies${kind}${dir}${action}`;
...
/* DENGE (v16.1). Canlar yukseltildi: oyuncunun gucu seviye+silahla 2-4 kat
 ...
 static readonly CAN:Record<number,number>={1:46,2:58,4:100,5:32,6:72,7:320,8:84,9:1,10:380,11:1,12:34};
...
/* Hasarlar ARTIK ACIK yaziliyor. Eski varsayilan (10+kind*3) tur numarasina
 ...
 static readonly HASAR:Record<number,number>={1:10,2:14,4:22,5:11,6:16,7:32,8:18,9:0,10:28,11:6,12:15};
 /** Takip hizi; tabloda yoksa 27. Bogulmus su icinde yurur gibi yavas. */
 static readonly HIZ:Record<number,number>={4:21,8:15,10:24,11:Engine.ISKELET_HIZ,12:27};
 ...
 static readonly GOLGE:Record<number,number>={1:9,2:8,4:8,5:11,6:8,7:18,8:9,9:5,10:10,11:8,12:12};
 ...
 static readonly SLASH_ISABET_KALIN=2.6;
 /** Yerdeki hammadde ganimetinin rengi [ic, kontur]. */
 static readonly HAM_RENK:Record<string,[string,string]>={
  kemikp:['#e6dcc4','#8f8365'],ag:['#d9d9d9','#7e7e7e'],zehirk:['#9fd47a','#4e6b3c'],
  kurum:['#6b6660','#332f2b'],pacavra:['#9a8b76','#544a3c'],post:['#b08a62','#5f4630'],
  et:['#c4706b','#6d3a37'],kanat:['#8a6a86','#4a394a'],celik:['#b9c0c7','#5e666d'],
 };
 ...
 static readonly DUSMAN_OLCEK:Record<number,number>={7:1.7,12:1.5};
 ...
 private kavanozPatla(x:number,y:number){
  ...
  for(const m of this.mobs){if(m.hp<=0)continue;const d=Math.hypot(m.x-x,m.y-y);
   if(d<34){const h=Math.round(26*(1-d/34)+14);m.hp-=h;m.hurt=Engine.HURT_SURE;m.burn=3;
 ...
 private iskeletKontrol(dt:number){
  ...
  for(const e of this.gomulu){
   ...
   /* Can TURDEN okunur: azman iskelet (12) burada da dogru cani almali -
      sabit 11 yazilinca azman 1 canla cikiyordu. */
   const max=Engine.CAN[e.kind]??1;
 ...
 /** Vurus kivilcimi: burst() yonsuz ve AYAK hizasindan doguyor, yani kilicin
  *  degdigi yerden degil yerden cikiyordu. Bu konik demet govde yuksekliginden
  *  ve darbe yonunde firliyor. Sayi ARTMADI (8) - daha fazlasi darbeyi degil
  *  bulanikligi artiriyor; duzelen sey YER ve YON. */
 private darbeKivilcim(m:Mob,ux:number,uy:number){
  for(let i=0;i<8;i++){
   const a=Math.atan2(uy,ux)+(Math.random()-.5)*1.0,h=70+Math.random()*60;
   this.particles.push({x:m.x-ux*4,y:m.y-10+(Math.random()-.5)*6,
    vx:Math.cos(a)*h,vy:Math.sin(a)*h-20,life:.16+Math.random()*.14,
    color:'#c76b5d',size:1+Math.random(),g:120});
  }
 }
 ...
 private burst(x:number,y:number,color:string,n:number){for(let i=0;i<n;i++)this.particles.push({x,y,vx:(Math.random()-.5)*55,vy:(Math.random()-.6)*55,life:.4+Math.random()*.4,color,size:1+Math.random()})}
  ...
  {const s=this.state;if(this.mesale>0)s.flags.mesaleKalan=String(Math.ceil(this.mesale));
   ...
   if(this.fireBurnCooldown<=0){
    ...
    for(const e of this.world.entities){
     ...
     for(const m of this.mobs){
      ...
      m.hp-=Engine.ATES_HASAR;m.hurt=Engine.HURT_SURE;m.burn=Math.max(m.burn,1.5);
   ...
   if(m.burn>0){m.burn-=dt;m.hp-=3*dt;if(m.id==='rauf'&&m.hp<=m.max*.18){this.raufDizCok();}else if(m.hp<=0){this.kill(m);continue;}}const dx=this.state.x-m.x,dy=this.state.y-m.y,d=Math.hypot(dx,dy)||1,visible=lineOfSight(this.world,m.x,m.y,this.state.x,this.state.y);if(m.kind===9){this.fener(m,d,dx,dy,dt);continue;}if(m.windup>0){m.windup-=dt;/* Iskelet HAMLE yapar: digerleri vurus hazirliginda cakili dururken iskelet oyuncuya dogru atilir. Olculdu: hamlesiz surumde skelet (38) oyuncudan (44) yavas oldugu icin windup biterken oyuncu menzilden cikiyordu ve 8 saniyelik bir gecis yalnizca 24 cana mal oluyordu - 'oldurmesi cok kolay' hissinin asil sebebi buydu. Hamle hizi oyuncunun ustunde ama yalnizca .26 sn surer. */if(Engine.ISKELET(m.kind))this.dusmanYurut(m,this.state.x,this.state.y,Engine.ISKELET_HAMLE,dt);if(m.windup<=0){/* Ses uzakliga gore kisiliyor: ekranin obur ucundaki bir yaratik yanindaki
    ...
    const range=m.kind===2?95:m.boss?34:m.kind===10?26:m.kind===11?Engine.ISKELET_SALDIRI_MENZIL:20;if(d>range){const v=(Engine.HIZ[m.kind]??27)*(m.zehir&&m.zehir>0?Engine.ZEHIR_YAVAS:1);this.dusmanYurut(m,this.state.x,this.state.y,v,dt);}if(d<=range&&m.cool<=0)m.windup=m.boss?.8:m.kind===2?.55:m.kind===10?.7:Engine.ISKELET(m.kind)?Engine.ISKELET_WINDUP:.4;}else if(!m.boss){// Oyuncu uzaktayken yaratiklar cakili duruyordu; magara olu gorunuyordu.
    ...
    if(hedef){
     ...
     if(hu>15){const v=42,ax=hx/hu*v*dt,ay=hy/hu*v*dt;let gitti=0;
      ...
      if(this.raufVur<=0){this.raufVur=.8;hedef.hp-=16;hedef.hurt=.18;
       this.float(hedef.x,hedef.y-12,'−16','#e2c98b');
  ...
  private sprite(key:string,x:number,y:number,frame=0,fw?:number,fh?:number,flip=false,scale=1,alpha=1,donder=0,capa?:number,flas=0){const im=this.images[key];if(!im?.naturalWidth)return;const w=fw||im.width/R,h=fh||im.height/R;const count=Math.floor(im.width/(w*R));const c=this.ctx;c.save();c.globalAlpha=alpha;const actor=key.startsWith('characters')||key.startsWith('enemies');/* Aktorler (oyuncu/mob) HAREKET ederken tam sayiya yuvarlama olmadan
   ...
   donduruyor) uyusmaz, dondurulen nesne editorde gorunenden kayik cikar. */if(donder){const my=(-anchor+h/2)*scale+(actor?0:6);c.translate(0,my);c.rotate(donder);c.translate(0,-my);}if(flip)c.scale(-1,1);const top=actor?-anchor*scale:-h*scale+6;c.drawImage(im,(frame%count)*w*R,0,w*R,h*R,-w*scale/2,top,w*scale,h*scale);/* VURUS FLASI: ayni kare 'lighter' ile ustune bir daha basiliyor. Ana
   tuvalde guvenli (offscreen tampon gerektirmiyor) ve siluet birebir
   korunuyor - piksel sanatinda dogru olan da bu, tam beyaz dolgu figuru
   siliyor. */if(flas>0){const eskiOp=c.globalCompositeOperation;c.globalCompositeOperation='lighter';c.globalAlpha=alpha*flas;c.drawImage(im,(frame%count)*w*R,0,w*R,h*R,-w*scale/2,top,w*scale,h*scale);c.globalCompositeOperation=eskiOp;c.globalAlpha=alpha;}c.restore()}
