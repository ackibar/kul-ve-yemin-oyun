...
for(const set of ['1sword','1bow','1balta'])for(const dir of ['D','U','S','DS','US'])for(const action of ['Idle','Walk','Attack','Hurt','Death']){jobs.push(this.img(`characters${set}${dir}${action}`,`/assets/characters/${set}/${dir}_${action}.png`));optional.push(dir==='DS'||dir==='US');}for(const z of ['haven','magara','disari','yikik','cistern'])jobs.push(this.img('bg_'+z,`/assets/arkaplan/${z}.png`));for(const z of ['haven','cistern','forge'])for(let i=0;i<16;i++)jobs.push(this.img(`wang_${z}_${i}`,`/assets/dungeon/wang/${z}/wang_${i}.png`));for(const [key,name]of [['fire','Fire1'],['lever','Lever1'],['trap','Spikes'],['portal','Trapdoor_D']])jobs.push(this.img(key,`/assets/dungeon/3%20Animated%20objects/${name}.png`));/* Sandik artik CraftPix setinden degil: oyunun paletinde uretilmis iki
   kareli kendi sheet'i (0 kapali, 1 acik). */jobs.push(this.img('chest','/assets/nesne/sandik.png'));for(const a of ["camasir", "fener", "fici", "kasa", "masa", "ocak", "odun", "raf", "sandik", "tabure", "tezgah", "yatak1", "yatak2"])jobs.push(this.img('nesne/'+a+'.png',`/assets/nesne/${a}.png`));for(const a of ['Tables/1.png','Tables/2.png','Chairs/1.png','Bookshelf/1.png','Bookshelf/2.png','Boxes/1.png','Boxes/2.png'])jobs.push(this.img(a,'/assets/dungeon/2%20Objects/'+a));mark(false);const result=await Promise.allSettled(jobs);
 ...
 private resetMobs(){this.mobs=this.world.enemies.filter(e=>!this.state.killed.includes(e.id)).map(e=>{const max=e.boss?300:(Engine.CAN[e.kind]??40)+(this.state.zone==='forge'?18:0);return {...e,hp:max,max,cool:1+Math.random(),windup:0,burn:0,hurt:0,homeX:e.x,homeY:e.y}});this.shots=[];this.particles=[];this.drops=[];this.activeTraps.clear();}
...
if(!walkable(this.world,s.x,s.y)){[s.x,s.y]=this.world.spawn;}this.camera={x:s.x-this.gorus.en/2,y:s.y-this.gorus.boy/2};this.resetMobs();this.attackTimer=this.dodgeTimer=this.dash=this.invulnerable=this.tonic=0;this.input={x:0,y:0,attack:false};this.audio.setZone(s.zone);this.emit();}
 ...
 private burst(x:number,y:number,color:string,n:number){for(let i=0;i<n;i++)this.particles.push({x,y,vx:(Math.random()-.5)*55,vy:(Math.random()-.6)*55,life:.4+Math.random()*.4,color,size:1+Math.random()})}
  ...
  {const r=this.state.equipment.ring,k=r?ITEMS[r].kurtarma||0:0;const en=stats(this.state).maxHp;
   if(k&&!this.halka&&this.state.hp>0&&this.state.hp<en/3){this.halka=true;this.state.hp=Math.min(en,this.state.hp+k);
    this.float(this.state.x,this.state.y-18,'YEMİN TUTTU','#f0d59a');this.audio.play('level');this.emit();}}for(const key of ['attackTimer','dodgeTimer','dash','invulnerable','tonic','bileme','merhem','gizli','slash','vurusPoz','trapCooldown']as const)this[key]=Math.max(0,this[key]-dt);
  ...
  *  koyu renkte 1px kaydirilarak dort yone cizilir, uzerine asil renk gelir. */
  ...
  for(const e of this.world.entities){if(e.type==='ceset'){this.sprite('ceset',e.x,e.y,0,32,32,false,OYUNCU_OLCEK*1.18);this.label(e.name||'',e.x,e.y-20,'#9d9384');continue;}if(e.type==='portal'){const gizli=e.asset==='gizli';/* Kapak sprite'i yalnizca boyali arka plani OLMAYAN mekanlarda: boyali
     ...
     zindan setinin izini birakiyordu. Isik halesi ve etiket kaliyor. */if(!gizli&&!bg?.naturalWidth)this.sprite('portal',e.x,e.y,0,22,32);if(e.id==='returnHaven'&&!this.state.killed.includes('warden'))continue;if(gizli){this.label(e.name!,e.x,e.y+13,'#c9bda4');continue;}const glow=c.createRadialGradient(e.x,e.y,1,e.x,e.y,22);glow.addColorStop(0,'#74c8ef30');glow.addColorStop(1,'#74c8ef00');c.fillStyle=glow;c.fillRect(e.x-22,e.y-22,44,44);this.label(e.name!,e.x,e.y+13,'#b6dbe9');}if(e.type==='trap')this.sprite('trap',e.x,e.y,this.trapActive(e)?4:0,17,17);}
