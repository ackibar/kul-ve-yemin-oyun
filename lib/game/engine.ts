    ...
    if(this.mobs.every(m=>m.hp<=0)&&gecen<Engine.DALGALAR.length){
     ...
     if(this.dalgaSayac<=0){
      ...
      for(const [kind,adet] of Engine.DALGALAR[gecen])for(let i=0;i<adet;i++){
       ...
       this.mobs.push({id:`dalga${gecen}_${n++}`,kind,x:kx*16+8+(Math.random()-.5)*26,
        y:ky*16+8+(Math.random()-.5)*10,hp:max,max,cool:.8+Math.random(),
        windup:0,burn:0,hurt:0,homeX:kx*16+8,homeY:ky*16+8});}
