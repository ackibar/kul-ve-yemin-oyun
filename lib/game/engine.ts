...
if(!walkable(this.world,s.x,s.y)){[s.x,s.y]=this.world.spawn;}this.camera={x:s.x-this.gorus.en/2,y:s.y-this.gorus.boy/2};this.resetMobs();this.attackTimer=this.dodgeTimer=this.dash=this.invulnerable=this.tonic=0;this.input={x:0,y:0,attack:false};this.audio.setZone(s.zone);this.emit();}
 ...
 useItem(id:ItemId){if(this.paused&&!['potion','tonic','bileme'].includes(id))return false;if(id==='potion'){if(this.state.hp>=stats(this.state).maxHp){this.notify('Canın zaten dolu.');return false;}if(!removeItem(this.state,id)){this.notify('Can iksirin kalmadı. Alf’ten alabilirsin.');return false;}this.state.hp=Math.min(stats(this.state).maxHp,this.state.hp+45);this.float(this.state.x,this.state.y-10,'+45','#8cdda5');}else if(id==='tonic'){if(!removeItem(this.state,id))return false;this.tonic=20;this.notify('Köz toniği: 20 saniye +8 saldırı.');}else if(id==='bileme'){if(!removeItem(this.state,id))return false;this.bileme=30;this.notify('Bileme taşı: 30 saniye %25 daha hızlı vuruş.');}else return false;this.audio.play('drink');this.save();this.emit();return true;}
 ...
 private kit(){const w=this.state.equipment.weapon;const v=ITEMS[w].menzilli?'bow':w==='yumruk'?'':'sword';/* Kontrol daima 'D' uzerinden: capraz yonde sheet olmayabilir ve set varken yedege dusmek yanlis olur. */return this.images['characters1'+v+'DIdle']?.naturalWidth?'characters1'+v:'characters1';}
 ...
 private move(p:{x:number;y:number},dx:number,dy:number,ben:string|null=null,mobDahil=false){
  ...
  this.tick+=dt;this.state.playtime+=dt;for(const key of ['attackTimer','dodgeTimer','dash','invulnerable','tonic','bileme','slash','vurusPoz','trapCooldown']as const)this[key]=Math.max(0,this[key]-dt);
