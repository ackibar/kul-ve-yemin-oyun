...
type Mob=EnemySpec&{hp:number;max:number;cool:number;windup:number;burn:number;hurt:number;homeX:number;homeY:number};
...
export class Engine{
 ...
 private floating:Floating[]=[];private shots:Shot[]=[];private camera={x:0,y:0};private slash=0;private ready=false;private saveStatus='';private trapCooldown=0;private fireBurnCooldown=0;
 ...
 constructor(canvas:HTMLCanvasElement,state:State,audio:GameAudio,onChange:(s:Snapshot)=>void,onEvent:(e:GameEvent)=>void){this.canvas=canvas;this.ctx=canvas?.getContext?.('2d')!;this.state=state;this.world=makeWorld(state.zone);this.audio=audio;this.onChange=onChange;this.onEvent=onEvent;this.gez.clear();this.resetMobs();this.loadAssets();this.camera={x:state.x-160,y:state.y-90};if(typeof window!=='undefined'){window.addEventListener('keydown',this.handleKeyDown);window.addEventListener('keyup',this.handleKeyUp);}this.loop=this.loop.bind(this);if(typeof requestAnimationFrame!=='undefined')this.raf=requestAnimationFrame(this.loop)}
 ...
 private resetMobs(){this.mobs=this.world.enemies.filter(e=>!this.state.killed.includes(e.id)).map(e=>{const max=e.boss?300:([0,34,42,56,80,24,72][e.kind])+(this.state.zone==='forge'?18:0);return {...e,hp:max,max,cool:1+Math.random(),windup:0,burn:0,hurt:0,homeX:e.x,homeY:e.y}});this.shots=[];this.particles=[];this.drops=[];this.activeTraps.clear();}
 setState(s:State){this.state=s;this.world=makeWorld(s.zone);
...
if(!walkable(this.world,s.x,s.y)){[s.x,s.y]=this.world.spawn;}this.camera={x:s.x-160,y:s.y-90};this.resetMobs();this.attackTimer=this.dodgeTimer=this.dash=this.invulnerable=this.tonic=0;this.input={x:0,y:0,attack:false};this.audio.setZone(s.zone);this.emit();}
 ...
 private kit(){const w=this.state.equipment.weapon;const v=w==='bow'?'bow':'sword';return this.images['characters1'+v+this.direction+'Idle']?.naturalWidth?'characters1'+v:'characters1';}
 toggleWeapon(){if(this.paused||!this.ready)return;const s=this.state;if(!s.inventory.bow){this.notify('Henüz yayın yok! Sarnıç kapağının yanındaki sandıkta bir yay var; Alf’ten de alabilirsin.');return;}const current=s.equipment.weapon;let next:ItemId='rusty';if(current==='bow'){const weapons=(Object.keys(s.inventory)as ItemId[]).filter(id=>ITEMS[id].kind==='weapon'&&id!=='bow');weapons.sort((a,b)=>(ITEMS[b].attack||0)-(ITEMS[a].attack||0));next=weapons[0]||'rusty';this.notify(`${ITEMS[next].name} kuşanıldı (Kılıç modu).`);}else{next='bow';const count=s.inventory.arrow||0;this.notify(`Avcı yayı kuşanıldı (Menzilli ok modu). Kalan ok: ${count}`);}s.equipment.weapon=next;s.hp=Math.min(s.hp,stats(s).maxHp);this.audio.play('select');this.save();this.emit();}
 dodge(){if(this.paused||this.dodgeTimer>0)return;this.dodgeTimer=stats(this.state).dodge;this.dash=.2;this.invulnerable=.36;this.audio.play('dodge');const kx=(this.keys.right?1:0)-(this.keys.left?1:0),ky=(this.keys.down?1:0)-(this.keys.up?1:0),inX=kx||this.input.x,inY=ky||this.input.y,n=Math.hypot(inX,inY);this.dashVector=n>.1?{x:inX/n,y:inY/n}:{x:this.direction==='S'?(this.flip?-1:1):0,y:this.direction==='U'?-1:this.direction==='D'?1:0};this.emit();}
 private attack(){if(this.attackTimer>0||this.paused)return;const s=this.state;const isBow=s.equipment.weapon==='bow';this.attackTimer=isBow?.38:.43;
   if(isBow){const arrowCount=s.inventory.arrow||0;if(arrowCount<=0){this.notify('Okun kalmadı! Sarnıç kapağının yanındaki sandıktan, Alf’ten veya diğer sandıklardan ok bulabilirsin.');return;}removeItem(s,'arrow');this.audio.play('swing');const damage=stats(s).attack+(this.tonic>0?8:0);let targets=this.mobs.filter(m=>Math.hypot(m.x-s.x,m.y-s.y)<160&&lineOfSight(this.world,s.x,s.y,m.x,m.y));targets.sort((a,b)=>Math.hypot(a.x-s.x,a.y-s.y)-Math.hypot(b.x-s.x,b.y-s.y));let vx=0,vy=0;if(targets[0]){const d=Math.hypot(targets[0].x-s.x,targets[0].y-s.y)||1;vx=(targets[0].x-s.x)/d*170;vy=(targets[0].y-s.y)/d*170;this.face(targets[0].x-s.x,targets[0].y-s.y);}else{const dirX=this.direction==='S'?(this.flip?-1:1):0;const dirY=this.direction==='U'?-1:this.direction==='D'?1:0;vx=dirX*170;vy=dirY*170;}this.shots.push({x:s.x,y:s.y-4,vx,vy,life:1.3,damage,isHero:true});this.save();this.emit();return;}
   ...
   let targets=this.mobs.filter(m=>Math.hypot(m.x-s.x,m.y-s.y)<38&&lineOfSight(this.world,s.x,s.y,m.x,m.y));targets.sort((a,b)=>Math.hypot(a.x-s.x,a.y-s.y)-Math.hypot(b.x-s.x,b.y-s.y));if(targets[0])this.face(targets[0].x-s.x,targets[0].y-s.y);for(const m of targets){const damage=stats(s).attack+(this.tonic>0?8:0);m.hp-=damage;m.hurt=.17;if(s.equipment.weapon==='ember')m.burn=3;if(s.equipment.weapon==='blood')s.hp=Math.min(stats(s).maxHp,s.hp+3);this.float(m.x,m.y-12,String(damage),'#ffdaa3');this.burst(m.x,m.y,'#c76b5d',8);this.audio.play('hit');const d=Math.hypot(m.x-s.x,m.y-s.y)||1;this.move(m,(m.x-s.x)/d*5,(m.y-s.y)/d*5);if(m.id==='rauf'&&m.hp<=m.max*.18){this.raufDizCok();}else if(m.hp<=0)this.kill(m);}this.emit();}
  private kill(m:Mob){
  if(m.id==='rauf'){this.raufDizCok();return;}
  if(this.state.killed.includes(m.id))return;this.state.killed.push(m.id);this.burst(m.x,m.y,m.boss?'#eeb559':'#a56a70',18);const xp=m.boss?210:([0,20,25,30,40,24,60][m.kind]);const gold=m.boss?60:5+m.kind*2;this.spawnDrop(m.x,m.y,'xp',xp);this.spawnDrop(m.x+(Math.random()-.5)*12,m.y+(Math.random()-.5)*12,'gold',gold);if(m.boss){this.notify('Kül Bekçisi yenildi. Kalbi al ve sığınağa dön.');addItem(this.state,'potion',2);}this.save();}
 ...
 private hurt(damage:number){if(this.invulnerable>0||this.state.hp<=0)return;const n=Math.max(2,damage-stats(this.state).defense);this.state.hp=Math.max(0,this.state.hp-n);this.invulnerable=.72;this.audio.play('hurt');this.float(this.state.x,this.state.y-14,'−'+n,'#ff8b89');this.burst(this.state.x,this.state.y,'#dc7777',7);if(this.state.hp<=0){this.paused=true;this.audio.play('death');this.onEvent({type:'death'});}this.emit();}
  ...
  private face(dx:number,dy:number){if(Math.abs(dx)>Math.abs(dy)){this.direction='S';this.flip=dx<0;}else if(Math.abs(dy)>.01){this.direction=dy<0?'U':'D';this.flip=false;}}
  ...
  *  koyu renkte 1px kaydirilarak dort yone cizilir, uzerine asil renk gelir. */
  ...
  private render(time:number){const c=this.ctx;const canvas=this.canvas;if(canvas.width!==1280||canvas.height!==720){canvas.width=1280;canvas.height=720;}c.setTransform(4,0,0,4,0,0);c.imageSmoothingEnabled=false;c.fillStyle='#0a111b';c.fillRect(0,0,320,180);const targetX=this.state.x-160,targetY=this.state.y-90;this.camera.x+=(targetX-this.camera.x)*.12;this.camera.y+=(targetY-this.camera.y)*.12;const cx=Math.round(this.camera.x),cy=Math.round(this.camera.y);c.translate(-cx,-cy);
  ...
  const isFloor=(tx:number,ty:number)=>ty>=0&&ty<this.world.h&&tx>=0&&tx<this.world.w&&this.world.tiles[ty][tx]===1;const y0=Math.floor(cy/16)-1,y1=Math.ceil((cy+180)/16)+2,x0=Math.floor(cx/16)-1,x1=Math.ceil((cx+320)/16)+2;
  ...
  const actors:({y:number;draw:()=>void})[]=this.world.entities.filter(e=>e.type!=='portal'&&e.type!=='trap').map(e=>({y:e.y,draw:()=>{
   ...
   if(e.type==='chest'){const opened=this.state.opened.includes(e.id);this.sprite('chest',e.x,e.y,opened?3:0,16,24,false,1,1);if(!opened){c.fillStyle='#e8c883';c.fillRect(e.x-1,e.y-20+Math.sin(time*3),2,2);}return;}
   if(e.type==='lever'){this.sprite('lever',e.x,e.y,this.state.flags.gateOpen?3:0,16,18);return;}
   if(e.type==='core'){if(this.state.inventory.core||this.state.ending)return;this.sprite('fire',e.x,e.y,Math.floor(time*8)%8,32,32, false,0.85);this.label('Kül kalbi',e.x,e.y+13,'#f5c677');return;}
   if(e.type==='npc'){c.fillStyle='#03091366';c.beginPath();c.ellipse(e.x,e.y+1,8*OYUNCU_OLCEK,2.6*OYUNCU_OLCEK,0,0,7);c.fill();if(e.id==='rauf'&&this.state.flags.rauf==='dizcokme'&&this.images.rauf_kneel?.naturalWidth){
  ...
  c.fillStyle=this.state.zone==='haven'?'#060e1924':'#070b1c42';c.fillRect(cx,cy,320,180);
  ...
  for(const s of this.shots){if(s.isHero){c.save();c.translate(s.x,s.y);c.rotate(Math.atan2(s.vy,s.vx));c.strokeStyle='#996d43';c.lineWidth=1.2;c.beginPath();c.moveTo(-6,0);c.lineTo(4,0);c.stroke();c.fillStyle='#e8f2fa';c.beginPath();c.moveTo(6,0);c.lineTo(2,-2);c.lineTo(2,2);c.closePath();c.fill();c.strokeStyle='#e64c4c';c.lineWidth=.8;c.beginPath();c.moveTo(-4,0);c.lineTo(-7,-2);c.moveTo(-4,0);c.lineTo(-7,2);c.stroke();c.restore();}else{c.fillStyle='#f2bd76';c.fillRect(s.x-2,s.y-2,4,4);c.fillStyle='#fff0bd';c.fillRect(s.x-1,s.y-1,2,2);}}
  ...
  const e=this.nearest();if(e&&!this.paused){c.strokeStyle='#e7c782';c.lineWidth=.6;c.beginPath();c.ellipse(e.x,e.y+4,10,4,0,0,7);c.stroke();}
